...
export default function DemoAccount() {
    ...
    function addFunds(amount) {
        ...
        setActivity(prev => [{ date: new Date().toISOString().slice(0, 10), type: "Deposit", amount }, ...prev]);
    ...
    return (
        <div className="flex flex-col h-full">
            <div className="bg-gray-300 text-gray-900 p-8 flex justify-center items-center">
                <div className="flex gap-12">
                    {/* Balance + Transactions Section */}
                    <div>
                        <div className="bg-gray-50 rounded-lg border p-8 mb-6 w-[430px]">
                            <div className="mb-4 text-center text-lg">Stocks, F&O balance</div>
                            <div className="text-center text-4xl font-bold mb-6">
                                ₹{virtualBalance.toLocaleString()}.<span className="text-2xl">00</span>
                            </div>
                            <hr className="mb-6" />
                            <div className="flex justify-between items-center py-3">
                                <span>Cash</span>
                                <span className="font-bold">₹{virtualBalance.toLocaleString()}.00</span>
                            </div>
                            {/* Pledge Section removed as requested */}
                        </div>
                        <div className="bg-white rounded-lg border p-6 mt-2 w-[430px]">
                            ...
                            <table className="w-full text-sm text-left border-collapse">
                                ...
                                <tbody>
                                    ...
                                    {activity.length === 0 &&
                                        <tr><td colSpan={3} className="text-center text-gray-400 py-2">No transactions yet</td></tr>}
                                </tbody>
                            </table>
                        </div>
                    </div>
                    {/* Add Money Section */}
                    <div>
                        <div className="bg-white rounded-lg border p-8 w-[430px] flex flex-col">
                            <div className="flex gap-12 mb-6">
                                <div className="text-green-600 font-semibold border-b-2 border-green-600 pb-1">Add money</div>
                                <div className="text-gray-500 cursor-not-allowed pb-1">Withdraw</div>
                            </div>
                            <label className="mb-2 block text-gray-600">Enter Amount</label>
                            <div className="flex items-center mb-4">
                                <span className="mr-2 text-green-600 text-lg font-semibold">₹</span>
                                <input
                                    type="number"
                                    min={500}
